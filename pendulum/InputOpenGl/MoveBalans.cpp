...
void  MoveBalans::calcFirstCoordinat() {
	subjectCoordinats.first = radius * cos(angleStart * PI / 180);
	subjectCoordinats.second = radius * sin(angleStart * PI / 180);
...
void  MoveBalans::calcCoordinat(const std::pair<int, int> pairIndex, 
	...
	double x = radius * cos(angleStart * PI / 180);
	double y = radius * sin(angleStart * PI / 180);
	if (counter == 1 && narrative) {
		x = x - (double)subjectCoordinats.first;
		y = (double)subjectCoordinats.second - y;

		coord1 = (subjectCoordinats.first + x);
		coord2 = (subjectCoordinats.second - y);
	}
	if (counter == 2 && narrative) {
		x =(double)subjectCoordinats.first - x;
		y = (double)subjectCoordinats.second - y;
		coord1 = (subjectCoordinats.first - x);
		coord2 = (subjectCoordinats.second - y);
	}
	coord1 *= pairIndex.first;
	coord2 *= pairIndex.second;
