...
class MoveBalans
{
	...
	// начальная координата сцепки
	std::pair<double, double>   couplCoordinats;
	// координата предмета
	std::pair<double, double> subjectCoordinats;
	// начальный угол подения
	double angleStart;
	double lastAngleStart;
	// напровление движение маятника
	// лево - true 
	// право - false
	bool narrative;
	// радиус
	...
	// на сколько изменилиськоординат
...
public:
	...
	std::pair<double, double> GetCoord() { return coord; };
	...
	// основная функция лдя подсчета всех данных
	void countingDataToMove(const short);
	// перехход угла по четвертям
	void transition();
